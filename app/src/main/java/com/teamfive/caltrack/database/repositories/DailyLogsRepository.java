package com.teamfive.caltrack.repository;
...
public class DailyLogsRepository {
    ...
    public LiveData<DailyLog> getOrCreateDailyLog(Date date) {
        MutableLiveData<DailyLog> dailyLogLiveData = new MutableLiveData<>();

        AppDatabase.getDatabaseWriteExecutor().execute(() -> {
            // Use the direct method to get the active goal
            Goals activeGoal = db.goalsDao().getActiveGoal();
            if (activeGoal != null) {
